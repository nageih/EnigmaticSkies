BlockEvents.rightClicked((event) => {
    ...
    if (item.id == 'arsdelight:vexing_bark' && block.id == 'malum:twisted_rock') {
        convertToBiome(event, 'minecraft:mushroom_fields', 'occultism:spirit_fire_flame', 10);
    }

    if (item.id == 'arsdelight:flourishing_bark' && block.id == 'naturesaura:infused_stone') {
        convertToBiome(event, 'minecraft:mangrove_swamp', 'supplementaries:green_flame', 10);
    }

    if (item.id == 'arsdelight:cascading_bark' && block.id == 'minecraft:prismarine_bricks') {
        convertToBiome(event, 'minecraft:warm_ocean', 'minecraft:soul_fire_flame', 10);
    }

    if (item.id == 'arsdelight:blazing_bark' && block.id == 'minecraft:polished_blackstone') {
